...
const processedBgImage = computed(() => {
  if (!proxy?.$imgHost) return ''

  if (!bgImage.value) {
    return proxy.$imgHost + 'ar_fallback.webp'
  }
  // 如果路径已经是完整的URL，直接返回
  if (bgImage.value.startsWith('http')) return bgImage.value
  // 如果路径已经包含 /static/，替换为相对路径
  if (bgImage.value.startsWith('/static/')) {
    return proxy.$imgHost + bgImage.value.replace('/static/', '')
  }
  // 返回带主机前缀的路径
  return proxy.$imgHost + bgImage.value
})
...
const resolvePoiIdForCompletion = (): string | null => {
  ...
  if (routeParams.value.poiId) {
    console.log(
      `✅ [降级策略] 通过路由参数获取到POI ID: ${routeParams.value.poiId}`
    )
  ...
  try {
    ...
    if (systemInfo.platform === 'devtools') {
      ...
      if (!hasImageError.value && isImageLoading.value) {
        bgImage.value = 'ar_fallback.webp'
...
const findNextNode = (nextId: string): ScriptNode | null => {
  ...
  const storyline =
    currentNPC.value.storylines?.[currentUserId] ||
    currentNPC.value.defaultStoryline
...
const transformNode = (
  node: ScriptNode
): ScriptNode & { speakerType?: string; task?: any; ending?: StoryEnding } => {
  ...
  if (node.type === 'task') {
    ...
    if (node.taskId) {
      ...
      if (!newNode.task && currentNPC.value?.commonTasks) {
        newNode.task = currentNPC.value.commonTasks.find(
          (t) => t.id === node.taskId
        )
        if (newNode.task) {
...
const getRandomCompletedNode = (
  ...
  // 过滤出已完成相关的节点
  const completedNodes = scriptNodes.filter(
    (node) => node.id && node.id.startsWith(completedNodePrefix)
  )
...
const buildInitialScript = () => {
  ...
  if (savedNodeId === 'act1_puzzle_success') {
    console.log(
      '🔄 检测到已完成解谜，自动跳过成功动画，跳转至: act1_pre_present'
    )
  ...
  if (!startNode && storyline?.startNodeId) {
    startNode =
      scriptNodes.find((node) => node.id === storyline.startNodeId) ||
      scriptNodes[0] ||
      null
  ...
  while (currentNode.jumpCondition && jumpCount < MAX_JUMPS) {
    ...
    // 1. 检查线索 (Clues)
    if (
      condition.requiredClue &&
      gameStore.inventory.clues.includes(condition.requiredClue)
    ) {
      shouldJump = true
    ...
    // 2. 检查印章 (Seals)
      ...
      condition.requiredSeal &&
      gameStore.inventory.seals.includes(condition.requiredSeal)
    ) {
      shouldJump = true
    ...
    // 3. 检查物品 (Items)
      ...
      gameStore.inventory.items.includes(condition.requiredItem)
    ) {
      shouldJump = true
    ...
    if (shouldJump) {
      ...
      if (nextNode) {
        console.log(
          `🔀 [剧情引擎] 自动跳转: ${currentNode.id} -> ${nextNode.id} (${reason})`
        )
  ...
  while (
    ...
    if (nextNode) {
      ...
      if (nextNode.jumpCondition) {
        ...
        if (shouldJump) {
          console.log(
            `🔀 [流式跳转] 跳过节点 ${nextNode.id} -> ${condition.nextId}`
          )
...
const loadNPCData = () => {
  ...
  console.log(
    ...
    targetNpcId,
    '(角色:',
    gameStore.currentUser?.id,
    ')'
  )

  // 从jieyang.npcs数组中查找对应的NPC（使用可能被覆盖的targetNpcId）
  currentNPC.value =
    jieyang.npcs.find((npc: NPC) => npc.id === targetNpcId) || null
  ...
  if (targetNpcId !== npcId) {
    ...
    console.log(
      '   当前NPC:',
      currentNPC.value.name,
      '(',
      currentNPC.value.title,
      ')'
    )
  }
...
const handleOptionSelected = (option: any) => {
  ...
  const isLeaveChoice =
    option.nextId === 'act1_leave_choice' || option.nextId === 'act1_leave'
...
const loadNextNode = (nextNodeId: string) => {
  ...
  while (
    ...
    if (subsequentNode) {
      // 🆕 如果遇到特殊节点（check、present_item），停止线性加载
      if (
        subsequentNode.type === 'check' ||
        subsequentNode.type === 'present_item'
      ) {
        break
      }
...
const handleLineChange = (line: any) => {
    ...
    switch (line.trigger) {
      ...
      case 'ending_normal':
        ...
        if (poiIdEnding && currentNPC.value) {
          ...
          console.warn(
            `[Trigger] 无法执行${line.trigger}任务完成：POI ID 或 NPC 数据缺失`
          )
        }
        break
  ...
  if (line.ending) {
    console.log(
      '[Auto-Trigger] 节点无显式 Trigger，但检测到 Ending 对象，执行自动完成逻辑'
    )
    ...
    if (poiIdFromEnding && currentNPC.value) {
      console.log(
        `[Auto-Trigger] 执行自动任务完成逻辑，POI ID: ${poiIdFromEnding}`
      )
      ...
      console.warn(
        '[Auto-Trigger] 无法执行自动任务完成：POI ID 或 NPC 数据缺失'
      )
    }
  }
...
const handleItemSelection = (selectedItemId: string) => {
  ...
  const isCorrect = gameStore.validatePresentation(
    selectedItemId,
    node.requiredItemId || ''
  )
...
const onImageError = () => {
  ...
  // 如果POI背景加载失败，使用黑色背景图
  if (bgImage.value !== 'ar_fallback.webp') {
    bgImage.value = 'ar_fallback.webp'
...
.ar-page .fallback-image[src*='ar_fallback.webp'] {
